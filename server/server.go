...
func (server *Server) Run(ctx context.Context, httpHandler http.Handler) error {
	...
	if server.TLS.Enabled {
		slog.InfoContext(ctx, "TLS is enabled")
...
func runAcmeChallengeServer(ctx context.Context, autocertManager *autocert.Manager) {
	...
	err := runCancelable(ctx, httpServer, func() error {
		slog.InfoContext(ctx, "HTTP (ACME challenge) listening on "+addr)
	...
	if err != nil {
		slog.ErrorContext(ctx, "ACME challenge server error", "error", err)
	}
}
...
func (server *Server) RunAutoCert(ctx context.Context, addr string, httpHandler http.Handler) error {
	...
	go runAcmeChallengeServer(ctx, autocertManager)
...
func (server *Server) RunManualTLS(ctx context.Context, addr string, httpHandler http.Handler) error {
	...
	err := runCancelable(ctx, httpServer, func() error {
		slog.InfoContext(ctx, "starting server", "address", "https://"+addr)
...
func (server *Server) RunUnsecured(ctx context.Context, addr string, httpHandler http.Handler) error {
	...
	err := runCancelable(ctx, httpServer, func() error {
		...
		slog.InfoContext(ctx, "starting server", "address", "http://"+addr)
...
func runCancelable(ctx context.Context, httpServer *http.Server, runFunc func() error) error {
	...
	case <-ctx.Done():
		...
		slog.InfoContext(ctx, "interrupt signal received")
